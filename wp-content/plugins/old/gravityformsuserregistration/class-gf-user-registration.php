...
class GF_User_Registration extends GFFeedAddOn {
	...
	public function process_feed( $feed, $entry, $form ) {
        ...
        if ($_REQUEST['user_id']){
            $entry['created_by'] = $_REQUEST['user_id'];
   ...
   //    echo '<pre>'; print_r ($at); echo '</pre>'; exit;
		...
		foreach ( $standard_meta as $meta_key ) {
            ...
            $user_info[$meta_key]=$value;
            $this->_user_info[$meta_key]=$value;
		}
    //    $xid = $this->curl_reg($user_info);
      //  $this->curl_reg_adv($user_info, $xid );
	...
	public static function maybe_prepopulate_form( $form ) {
		...
		if ( ! $feed ) {
           ...
           if ($_REQUEST['user_id']){
			...
			$user_id = gf_apply_filters( 'gform_user_registration_update_user_id', $form['id'], $user_id, false, $form, $feed );
	...
	public function update_buddypress_data( $user_id, $feed, $entry ) {
        ...
        foreach($bp_data as $bp_d){

            $xprofile = $bp_d['field'];
            $user_profile_fields[$xprofile->name] = $bp_d['value'];
       ...
       if (in_array($user_profile_fields['Member type'], $athlete)){

          /*
           echo "<pre>";
            echo $user_id;

           $data = bp_get_member_profile_data( array( 'field'=>'SSSFiD','user_id' => $user_id ) );// use bp_get_members_user_id() or just remove that argument if you are inside the members loop(e.g. on members directory).
           $data =  BP_XProfile_ProfileData::get_value_byid( '25', $user_id );
           print_r ($data);
           echo "test";exit;
            */

           $xid = $this->curl_reg($this->_user_info);

           $this->curl_reg_adv($this->_user_info, $xid, $user_profile_fields );
       }elseif(in_array($user_profile_fields['Member type'], $coache)){
           $xid = $this->curl_reg_coach($this->_user_info, $user_profile_fields);

        //   $this->curl_reg_adv_coach($this->_user_info, $xid, $user_profile_fields );
        ...
        xprofile_set_field_data('SSSFiD', $user_id,   $xid);
...
//create cURL connection
        $curl_connection =
            curl_init('http://shot.sssfonline.com/shot/bin/athletes/add.asp');
...
//close the connection
    ...
    public function curl_reg_adv($user, $xid, $user_profile_fields ){
        ...
        $user_profile_fields[$map_keys['bdt_1']] =10;
        $user_profile_fields[$map_keys['bdt_2']] =10;
        $user_profile_fields[$map_keys['bdt_3']] = 2001;
        $user_profile_fields[$map_keys['stt']] = 43;
        $data = array(
            'fnm' => $user['first_name'],
            'lnm' => $user['last_name'],
            'eid' => '556',
            ...
            'org' => 'C',
            'fct' => 'UA',
            'ad1'  => $user_profile_fields[$map_keys['ad1']],
            'cty'  => $user_profile_fields[$map_keys['cty']],
            'stt' => $user_profile_fields[$map_keys['stt']],
            'zip' => $user_profile_fields[$map_keys['zip']],
            'hp'  => $user_profile_fields[$map_keys['hp']],
            'eml' => $user_profile_fields[$map_keys['eml']],
            'gnd' => $user_profile_fields[$map_keys['gnd']],
            'eth' => $user_profile_fields[$map_keys['eth']],
            'tshrt' => 'M',
            'bdt_1' => $user_profile_fields[$map_keys['bdt_1']],
            'bdt_2' => $user_profile_fields[$map_keys['bdt_2']],
            'bdt_3' => $user_profile_fields[$map_keys['bdt_3']],
            'sch'  => $user_profile_fields[$map_keys['sch']],
            'grad' => $user_profile_fields[$map_keys['grad']],
            'prn'  => $user_profile_fields[$map_keys['prn']],
            'en1'  => $user_profile_fields[$map_keys['en1']],
            'eh1'  => $user_profile_fields[$map_keys['eh1']],
            'em1'  =>$user_profile_fields[$map_keys['em1']],
            'nte'  => '',
            'dnr'  => $user_profile_fields[$map_keys['dnr']],
            'ata' => $user_profile_fields[$map_keys['ata']],
            'nssa'  =>$user_profile_fields[$map_keys['nssa']],
            'nsca' => $user_profile_fields[$map_keys['nsca']],
            'usas' => $user_profile_fields[$map_keys['usas']],
            'ngb_1' => '173',
            'class_1' => '0',
            'ngb_2'  => '171',
            'class_2' => '0',
...
//create cURL connection
        $curl_connection =
            curl_init('http://shot.sssfonline.com/shot/bin/athletes/athlete.asp');
...
//close the connection
  ...
  //  exit;
    ...
    public function curl_reg_coach($user, $user_profile_fields){
      ...
      //  print_r ($user);
        ...
        $data = array(
            'fn' => $user['first_name'],
            'ln' => $user['last_name'],
            'adr' => $user_profile_fields['Address'],
            'cty' => $user_profile_fields['City'],
            'stt' => $this->states($user_profile_fields['State']),
            'zip' => $user_profile_fields['ZIP'],
            'gnd' => $user_profile_fields['Gender'],
            'eml' => $user_profile_fields['Email'],
            'pos' => $user_profile_fields['Coach Type'],
            'pid' => '11527',
            'peid' => '14382',
            'eid' => '556',
            'mid' => '5',
...
//create cURL connection
        $curl_connection =
            curl_init('http://shot.sssfonline.com/shot/bin/positions/add.asp');
...
//close the connection
    ...
    public function curl_reg_adv_coach($user, $xid, $user_profile_fields ){
...
//close the connection
    ...
    public function parser_curl($content){
    ...
    public static function states($st){
