...
class GFAuthorizeNetcustom extends GFPaymentAddOn {

	protected $_version = GF_AUTHORIZENET_VERSION;
	protected $_min_gravityforms_version = '1.9.12';
	protected $_slug = 'gravityformsauthorizenet_1';
	protected $_path = 'gravityformsauthorizenet_1/authorizenet.php';
	...
	protected $_capabilities = array(
		'gravityforms_authorizenet_1',
		'gravityforms_authorizenet_1_uninstall',
		'gravityforms_authorizenet_1_plugin_page'
	);

	// Permissions
	protected $_capabilities_settings_page = 'gravityforms_authorizenet_1';
	protected $_capabilities_form_settings = 'gravityforms_authorizenet_1';
	protected $_capabilities_uninstall = 'gravityforms_authorizenet_1_uninstall';
	protected $_capabilities_plugin_page = 'gravityforms_authorizenet_1_plugin_page';
	...
	public static function get_instance() {
		if ( self::$_instance == null ) {
			self::$_instance = new GFAuthorizeNetcustom();
		}
	...
	public function init_ajax() {
		...
		add_action( 'wp_ajax_gf_dismiss_authorizenet_1_menu', array( $this, 'ajax_dismiss_menu' ) );

	}

	public function maybe_create_menu( $menus ) {
		$current_user              = wp_get_current_user();
		$dismiss_authorizenet_menu = get_metadata( 'user', $current_user->ID, 'dismiss_authorizenet_1_menu', true );
	...
	public function ajax_dismiss_menu() {
		...
		$current_user = wp_get_current_user();
		update_metadata( 'user', $current_user->ID, 'dismiss_authorizenet_1_menu', '1' );
	}
	...
	public function temporary_plugin_page() {
		...
		<script type="text/javascript">
			function dismissMenu() {
				...
				jQuery.post(ajaxurl, {
						action: 'gf_dismiss_authorizenet_1_menu'
					},
					function (response) {
		...
		<div class="wrap about-wrap">
			...
			<div
				class="about-text"><?php esc_html_e( 'Thank you for updating! The new version of the Gravity Forms Authorize.Net Rusty  Add-On makes changes to how you manage your Authorize.Net integration.', 'gravityformsauthorizenet' ) ?></div>
			<div class="changelog">
				<hr/>
				...
				<form method="post" id="dismiss_menu_form" style="margin-top: 20px;">
					<input type="checkbox" name="dismiss_authorizenet_1_menu" value="1" onclick="dismissMenu();">
	...
	public function authorize( $feed, $submission_data, $form, $entry ) {
		...
		$transaction = apply_filters( 'gform_authorizenet_1_transaction_pre_authorize', $original_transaction, $form_data, $config, $form );
		$transaction = apply_filters( 'gform_authorizenet_1_transaction_pre_capture', $transaction, $form_data, $config, $form, $entry );
		...
		if ( ! $transaction ) {
			...
			$auth_amount = apply_filters( 'gform_authorizenet_1_amount_pre_authorize', $form_data['amount'] + floatval( $form_data['fee_amount'] ), $original_transaction, $form_data, $config, $form, $entry );
			...
			if ( $response->approved || $response->held ) {
				...
				$error_message     = apply_filters( 'gform_authorizenet_1_validation_message', $error_message, $validation_result, $_POST, $response, 'aim' );
		...
		//deprecated
		$transaction = apply_filters( 'gform_authorizenet_1_before_single_payment', $transaction, $form_data, $config, $form );
	...
	public function process_capture( $authorization, $feed, $submission_data, $form, $entry ) {
		 ...
		 * @deprecated
		 */
		do_action( 'gform_authorizenet_1_post_capture', rgar( $authorization, 'is_authorized' ), rgars( $authorization, 'captured_payment/amount' ), $entry, $form, $this->_args_for_deprecated_hooks['config'], $this->_args_for_deprecated_hooks['aim_response'] );
	...
	public function subscribe( $feed, $submission_data, $form, $entry ) {
		...
		if ( ! empty( $fee_amount ) && $fee_amount > 0 ) {
			...
			$transaction = apply_filters( 'gform_authorizenet_1_transaction_pre_capture_setup_fee', $transaction, $form_data, $config, $form, $entry );
		...
		if ( $setup_fee_result ) {
			...
			if ( has_filter( 'gform_authorizenet_1_subscription_pre_create' ) ) {
				$this->log_debug( __METHOD__ . '(): Executing functions hooked to gform_authorizenet_subscription_pre_create.' );
				$subscription = apply_filters( 'gform_authorizenet_1_subscription_pre_create', $subscription, $form_data, $config, $form, $entry );
			}

			//deprecated
			if ( has_filter( 'gform_authorizenet_1_before_start_subscription' ) ) {
				$this->log_debug( __METHOD__ . '(): Executing functions hooked to gform_authorizenet_before_start_subscription.' );
				$subscription = apply_filters( 'gform_authorizenet_1_before_start_subscription', $subscription, $form_data, $config, $form );
			}
			...
			if ( $arb_response->isOk() ) {
				...
				$validation_result = $this->get_valiation_result( $form, $error_message );
				$error_message     = apply_filters( 'gform_authorizenet_1_validation_message', $error_message, $validation_result, $_POST, $arb_response, 'arb' );
	...
	public function process_subscription( $authorization, $feed, $submission_data, $form, $entry ) {
		 ...
		 * @deprecated
		 */
		do_action( 'gform_authorizenet_1_post_create_subscription', $authorization['subscription']['is_success'], $this->_args_for_deprecated_hooks['arb_subscription'], $this->_args_for_deprecated_hooks['arb_response'], $entry, $form, rgars($authorization, 'subscription/config') );
		do_action( 'gform_authorizenet_1_after_subscription_created', $authorization['subscription']['subscription_id'], $authorization['subscription']['amount'], rgars( $authorization, 'subscription/captured_payment/amount' ) );
	...
	public function cancel( $entry, $feed ) {
		...
		if ( $cancel_response->isOk() ) {
			 ...
			 */
			do_action( 'gform_subscription_1_canceled', $entry, $feed, $entry['transaction_id'], 'authorize.net' );
				...
				foreach ( $results as $result ) {
					...
					switch ( strtolower( $status ) ) {
						case 'active' :
							...
							//deprecated
							do_action( 'gform_authorizenet_1_post_recurring_payment', $subscription_id, $entry, $new_payment_amount, $payment_count );
							do_action( 'gform_authorizenet_1_after_recurring_payment', $entry, $subscription_id, $subscription_id, $new_payment_amount );
						...
						case 'expired' :
							...
							//deprecated
							do_action( 'gform_authorizenet_1_post_expire_subscription', $subscription_id, $entry );
							do_action( 'gform_authorizenet_1_subscription_ended', $entry, $subscription_id, $transaction_id, $new_payment_amount );
						...
						case 'suspended':
							...
							//deprecated
							do_action( 'gform_authorizenet_1_post_suspend_subscription', $subscription_id, $entry );
							do_action( 'gform_authorizenet_1_subscription_suspended', $entry, $subscription_id, $transaction_id, $new_payment_amount );
						 ...
						 * @deprecated
						 */
							do_action( 'gform_authorizenet_1_post_cancel_subscription', $subscription_id, $entry );
							do_action( 'gform_authorizenet_1_subscription_canceled', $entry, $subscription_id, $transaction_id, $new_payment_amount );
