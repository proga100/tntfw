    ...
    public function queryBasedConstruct($query, $queryParams = array(), $wdtParameters = array(), $init_read = false) {
        ...
        if ($this->serverSide()) {
            ...
            if (!isset($_POST['draw'])) {
             ...
             //   print_r ($parsedSearch);
                $search=null; // rusty code
                if ($search) {
                    if ($isMySql) {
        ...
        if ($this->isAjaxReturn()) {
            ...
            if (!empty($sumColumns) || !empty($avgColumns) || !empty($maxColumns) || !empty($minColumns)) {
               ...
               // print_r ( $output );
                ...
                //print_r ($_POST['columns']['0'] ['search']['value']);
                    // rusty code
                   $serach_0 =  $_POST['columns']['0'] ['search']['value'];

                     //   echo $serach_0 ;

                    $serach_1 =  $_POST['columns']['3'] ['search']['value'];


                          $dt_show =0;
                foreach ($output['data'] as $k=>$column){

                        if ((!empty($serach_0)) && (!empty( $serach_1))){
                            $dt_show = 1;


                            if(($serach_0 ==$column[0]) && ( $serach_1==$column[3])){
                                $datas[] = $column;

                            }

                        }elseif ((!empty($serach_0)) && (empty($serach_1))){
                            $dt_show=1;
                           if ($serach_0==$column[0]) $datas[]=$column;


                        }elseif ((empty($serach_0)) && (!empty($serach_1))){
                            $dt_show=1;
                            if ($serach_1==$column[3]) $datas[]=$column;
          ...
          // print_r ( $output );
			...
			if( is_user_logged_in() ) {
			  ...
			  if(in_array('coache', $role)){
				  $team = xprofile_get_field_data( 'Team', $user->ID );  
				  $teamwhere = " AND team.value='". $team."'"; 
			  }
		
			
	
			
			$qr = explode('LIMIT', $query);
			
			
			$query = $qr[0].$teamwhere." LIMIT ".$qr[1];
			// print_r ( $query );
