    ...
    if (!(Connection::isSeparate($tableData->connection))) {
        ...
        if ($idVal != '0') {
            ...
            if (!$res) {
                if (!empty($wpdb->last_error)) {
                    $returnResult['error'] = __('There was an error trying to update the row12! Error: ', 'wpdatatables') . $wpdb->last_error;
                } else {
        ...
        if ($idVal != '0') {
            ...
            if ($sql->doQuery($query)) {
                ...
                if ($sql->getLastError() !== '') {
                    $returnResult['error'] = __('There was an error trying to update the row13! Error: ', 'wpdatatables') . $sql->getLastError();
                } else {
                    $returnResult['success'] = $idVal;
                }
            }
        ...
        foreach ($cellsData as $cellData) {
            ...
            if (!key_exists($idColumnKey, $cellData)) {
                ...
                if (Connection::isSeparate($tableData->connection)) {
                    ...
                    if ($res === false) {
                        $returnResult['error'] = __('There was an error trying to update the row11! Error: ', 'wpdatatables') . $wpdb->last_error;
                    } else {
